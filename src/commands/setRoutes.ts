import { searchRead, write } from '../api/odoo';

function parseCehNumber(name: string): number | null {
  // Matches: "Цех №7", "Цех №2-1", "Цех №10" — captures main цех number only
  const m = name.match(/[Цц]ех\s*[№#]?\s*(\d+)/i);
  return m ? parseInt(m[1], 10) : null;
}

function findRoute(routes: Array<{ id: number; name: string }>, ...keywords: string[]) {
...
async function run() {
  ...
  if (!mfgRoute) throw new Error('Маршрут Manufacture не знайдено — перевір назви маршрутів вище');
  if (!mtoRoute) throw new Error('Маршрут MTO (ВНЗ) не знайдено — перевір назви маршрутів вище');
  ...
  const workcenters = await searchRead<{ id: number; name: string }>('mrp.workcenter', [], ['id', 'name']);
  ...
  const boms = await searchRead<{ id: number; product_tmpl_id: [number, string] }>(
    'mrp.bom', [], ['id', 'product_tmpl_id']
  );
  const bomToTmpl = new Map<number, number>();
  for (const bom of boms) bomToTmpl.set(bom.id, bom.product_tmpl_id[0]);

  // 5. Operations → max цех number per template
  const ops = await searchRead<{
    bom_id: [number, string];
    workcenter_id: [number, string];
  }>('mrp.routing.workcenter', [], ['bom_id', 'workcenter_id']);

  const tmplMaxCeh = new Map<number, number>();
  for (const op of ops) {
    ...
    if (ceh > prev) tmplMaxCeh.set(tmplId, ceh);
  }

  // 4. Load all templates that have BOMs
  const tmplIds = [...new Set(boms.map(b => b.product_tmpl_id[0]))];
  const templates = await searchRead<{ id: number; name: string; route_ids: number[] }>(
  ...
  for (const tmpl of templates) {
    ...
    if (maxCeh === undefined)  unknownTemplates.push(tmpl);
    else if (maxCeh <= 6)      mtsTemplates.push(tmpl);
    else                       mtoTemplates.push(tmpl);
  }
