...
async function main() {
  ...
  const mos = await searchRead<{
    ...
    'mrp.production',
    search ? [['product_id.name', 'ilike', search]] : [],
    ['id', 'name', 'product_id', 'state', 'move_raw_ids']
  );
  ...
  for (const mo of mos) {
    ...
    const moves = await searchRead<{
      ...
      ['id', 'product_id', 'product_qty', 'state']
    );
    ...
    const linkedMos = await searchRead<{
      ...
      ['id', 'name', 'product_id', 'state', 'origin']
    );
  ...
  const boms = await searchRead<{
    ...
    'mrp.bom',
    [['product_tmpl_id.name', 'ilike', search]],
    ['id', 'product_tmpl_id', 'product_id', 'code', 'bom_line_ids']
  );
  ...
  for (const bom of boms) {
    ...
    const templates = await searchRead<{
      ...
      'product.product',
      [['id', 'in', tmplIds]],
      ['id', 'name', 'route_ids', 'type']
    );
    const tmplMap = new Map(templates.map(t => [t.id, t]));

    const routeNames = await searchRead<{ id: number; name: string }>(
      'stock.route', [], ['id', 'name']
    );
