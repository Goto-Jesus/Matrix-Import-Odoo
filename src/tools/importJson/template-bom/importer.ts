...
import type { BomEntry, AttributeVal, ComponentEntry } from '../../docToJson/types';
...
async function loadRoutes(): Promise<void> {
  ...
  const routes = await searchRead<{ id: number; name: string }>('stock.route', [], ['id', 'name']);
  const find = (...kws: string[]) =>
    routes.find(r => kws.some(kw => r.name.toLowerCase().includes(kw.toLowerCase())));

  const mfg = find('manufactur', 'виробн');
  const mto = find('внз', 'make to order', 'replenish on order', 'поповнит');

  if (!mfg) throw new Error('Маршрут Manufacture не знайдено. Перевір stock.route в Odoo.');
  if (!mto) throw new Error('Маршрут MTO/ВНЗ не знайдено. Перевір stock.route в Odoo.');
 ...
 */
async function applyRoutes(templateId: number, templateName: string): Promise<void> {
...
async function applyProductTypes(boms: BomEntry[]): Promise<void> {
  ...
  const resolve = (names: string[]) =>
    names.map(n => nameToId.get(n)).filter((id): id is number => id !== undefined);
  ...
  if (finalIds.length > 0) {
    await write('product.template', finalIds, { sale_ok: true, purchase_ok: false });
    console.log(`[type] ФІНАЛЬНІ (продаж ✓ / закупка ✗): ${finalIds.length}`);
    for (const n of finalNames) console.log(`  - ${n}`);
  }
  if (semiIds.length > 0) {
    await write('product.template', semiIds, { sale_ok: false, purchase_ok: false });
    console.log(`\n[type] НАПІВФАБРИКАТИ (тільки виробництво): ${semiIds.length}`);
    for (const n of semiNames) console.log(`  - ${n}`);
  }
  if (rawIds.length > 0) {
    await write('product.template', rawIds, { sale_ok: false, purchase_ok: true });
 ...
 */
const RAW_MATERIAL_CATEGORY: Array<[keyword: string, categoryCompleteName: string]> = [
  ['Скотч', 'Упаковка'],
  ['Плівка', 'Упаковка'],
  ['Картон', 'Упаковка'],
  ...
  ['Холофайбер', 'Сировина / Наповнювачі'],
...
async function applyProductCategories(boms: BomEntry[]): Promise<void> {
  ...
  for (const entry of boms) {
    if (!firstOpForName.has(entry.product.templateName) && entry.operations[0]) {
  ...
  type Assignment = { name: string; catId: number; catName: string; source: 'final' | 'raw' | 'semi' };
  ...
  for (const name of allNames) {
    ...
    if (isFinal) {
      assignments.push({ name, catId: finalCatId, catName: FINAL_PRODUCT_CATEGORY, source: 'final' });
      continue;
    }
    ...
    if (!isManufactured) {
      ...
      if (rawCatName) {
        ...
        if (catId) assignments.push({ name, catId, catName: rawCatName, source: 'raw' });
  ...
  const templates = await searchRead<{ id: number; name: string; categ_id: [number, string] | false }>(
    'product.template',
    [['name', 'in', [...allNames]]],
    ['id', 'name', 'categ_id'],
  );
  ...
  for (const a of assignments) {
    ...
    if (!t) { notInOdoo.push(a.name); continue; }
    const currentId = t.categ_id ? t.categ_id[0] : 0;
    if (currentId === a.catId) { alreadyOk.push(a); continue; }
  ...
  if (missingCategory.length > 0) {
    ...
    for (const m of missingCategory) console.log(`  ? "${m.catName}" для ${m.name}`);
  }
...
function applyModelTransform(entries: BomEntry[]): BomEntry[] {
  const filterAttrs = (attrs: AttributeVal[]): AttributeVal[] =>
    attrs.filter(a => a.attributeName !== 'Модель' && !a.attributeName.includes('❌'));

  const filterExpand = (expand: Record<string, string[]>): Record<string, string[]> =>
    Object.fromEntries(
      Object.entries(expand).filter(([key]) => key !== 'Модель' && !key.includes('❌'))
    );
  ...
  return entries.map(entry => {
    ...
    if (!modelAttr) {
      ...
      return hasChanged ? { ...entry, components: transformedComponents } : entry;
    }
...
async function resolvePtavIds(templateId: number, attrs: AttributeVal[]): Promise<number[]> {
...
async function resolveIfAttrPtavIds(
  ...
  return resolvePtavIds(
    templateId,
    Object.entries(ifAttr).map(([attributeName, value]) => ({ attributeName, value }))
  );
}
...
export async function importTemplateBomGroup(
  ...
  if (found) {
    ...
    const resolved = await ensureVariantFromAttrs(templateName, [], false, false);
    if (!resolved) {
  ...
  const [existingBom] = await searchRead<{ id: number }>(
    'mrp.bom',
    [['product_tmpl_id', '=', tmplId], ['product_id', '=', false], ['code', '=', bomCode]],
    ['id'], 1
  );
  if (existingBom) {
    ...
    for (const comp of entry.components) {
      ...
      if (placeholderAttrs.length === 0) {
        const compResolved = await ensureVariantFromAttrs(
          comp.templateName, fixedAttrs, comp.isService ?? false, true
        );
        if (!compResolved) {
        ...
        const lineId1 = await create('mrp.bom.line', {
          ...
          ...(entryPtavIds.length > 0
            ? { bom_product_template_attribute_value_ids: [[6, 0, entryPtavIds]] }
            : {}),
        });
      ...
      const dims = placeholderAttrs
        .map(a => ({ key: a.attributeName, values: expand[a.attributeName] ?? [] }))
      ...
      for (const combo of combos) {
        const resolvedAttrs: AttributeVal[] = [
          ...fixedAttrs,
          ...placeholderAttrs
            .map(a => ({ attributeName: a.attributeName, value: combo[a.attributeName] ?? a.value }))
            .filter(a => !a.value.includes('%')),
        ];

        const compResolved = await ensureVariantFromAttrs(
          comp.templateName, resolvedAttrs, comp.isService ?? false, true
        );
        if (!compResolved) continue;

        const comboPtavIds = (
          await Promise.all(
            Object.entries(combo).map(([attrName, value]) => getPtavId(tmplId, attrName, value))
          )
 ...
 */
export async function importAllTemplateBoms(boms: BomEntry[], label?: string): Promise<void> {
  resetSession();
  ...
  const preSeedNames = [...new Set(transformedBoms.map(e => e.product.templateName))];
  const preSeedTemplates = await searchRead<{ id: number; name: string; categ_id: [number, string] | false }>(
    'product.template',
