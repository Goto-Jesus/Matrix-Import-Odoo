...
import { resolveProduct, getOrCreateWorkcenter } from './product';
import type { BomDef } from './types';

function bomLabel(def: BomDef): string {
 ...
 */
export async function createBom(def: BomDef): Promise<number | null> {
  const label = bomLabel(def);
  const reference = def.reference ?? def.variants?.join(' / ') ?? '';

  // 1. Знайти вихідний варіант товару
  const resolved = await resolveProduct(def.product, def.variants);
  if (!resolved) {
  ...
  const [existing] = await searchRead<{ id: number }>(
    'mrp.bom',
    [['product_id', '=', resolved.variantId], ['code', '=', reference]],
    ['id'], 1
  );
  if (existing) {
  ...
  for (let i = 0; i < def.operations.length; i++) {
    ...
    const wcId = await getOrCreateWorkcenter(op.workcenter);
  ...
  for (let i = 0; i < def.components.length; i++) {
    ...
    const compResolved = await resolveProduct(comp.product, comp.variants);
    if (!compResolved) {
 ...
 */
export async function createAllBoms(boms: BomDef[]): Promise<void> {
