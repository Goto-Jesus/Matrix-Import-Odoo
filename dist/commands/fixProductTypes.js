...
async function run() {
    ...
    const boms = await (0, odoo_1.searchRead)('mrp.bom', [], ['product_tmpl_id']);
    const manufacturedIds = new Set(boms.map(b => b.product_tmpl_id[0]));
    // 2. Component templates: bom.line.product_id → variant → template
    const bomLines = await (0, odoo_1.searchRead)('mrp.bom.line', [], ['product_id']);
    const componentVariantIds = [...new Set(bomLines.map(l => l.product_id[0]))];
    let componentTemplateIds = new Set();
    if (componentVariantIds.length > 0) {
        const variants = await (0, odoo_1.searchRead)('product.product', [['id', 'in', componentVariantIds]], ['id', 'product_tmpl_id']);
        componentTemplateIds = new Set(variants.map(v => v.product_tmpl_id[0]));
    }
    // 3. All templates
    const templates = await (0, odoo_1.searchRead)('product.template', [], ['id', 'name', 'sale_ok', 'purchase_ok', 'type']);
    // 4. Classify
    const groups = { final: [], semi: [], raw: [] };
    const skipped = [];
    for (const t of templates) {
        ...
        if (isM && !isC && !looksLikeSemi(t.name))
            groups.final.push(t);
        else if (isM)
            groups.semi.push(t);
        else if (isC)
    ...
    console.log(LINE);
    const updatesByKey = { final: [], semi: [], raw: [] };
    let totalToUpdate = 0;
    ...
    for (const key of Object.keys(CATEGORIES)) {
        ...
        if (list.length === 0)
            continue;
        const target = CATEGORIES[key];
        await (0, odoo_1.write)('product.template', list.map(t => t.id), {
