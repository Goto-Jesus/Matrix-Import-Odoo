...
async function main() {
    ...
    const attrs = await (0, odoo_1.searchRead)('product.attribute', [['name', 'in', [sourceAttr, targetAttr]]], ['id', 'name', 'value_ids']);
    const source = attrs.find((a) => a.name === sourceAttr);
    const target = attrs.find((a) => a.name === targetAttr);
    if (!source)
        throw new Error(`Атрибут "${sourceAttr}" не знайдено в Odoo`);
    if (!target)
        throw new Error(`Атрибут "${targetAttr}" не знайдено в Odoo`);
    console.log(`\n============================================================`);
    ...
    const sourceValues = await (0, odoo_1.searchRead)('product.attribute.value', [['attribute_id', '=', source.id]], ['id', 'name']);
    const existingValues = await (0, odoo_1.searchRead)('product.attribute.value', [['attribute_id', '=', target.id]], ['id', 'name']);
    const existingNames = new Set(existingValues.map((v) => v.name));
    const toCreate = sourceValues.filter((v) => !existingNames.has(v.name));
    const skipped = sourceValues.filter((v) => existingNames.has(v.name));
    ...
    for (const val of toCreate) {
        try {
            await (0, odoo_1.create)('product.attribute.value', { name: val.name, attribute_id: target.id });
