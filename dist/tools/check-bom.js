...
exports.runBomCheck = runBomCheck;
...
function isOutputLine(t) {
    if (ANY_QTY_RE.test(t))
        return false; // has qty → it's a component
...
function hasConditionalTag(rawLine) {
    return /\/\/\s*@/.test(rawLine);
}
function runBomCheck(content, fileName, applyTodos = true) {
    console.log(`\nBOM: ${fileName}`);
    const lines = content.split("\n");
    const issues = [];
    const fixes = [];
    const pendingTodos = new Map(); // lineIdx → todo messages
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (const svcName of SERVICE_ITEMS) {
            if (!trimmedLine.includes(svcName))
                continue;
            if (trimmedLine.startsWith("(Послуга)"))
                break; // вже правильно
