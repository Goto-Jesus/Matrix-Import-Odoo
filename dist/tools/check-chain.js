...
exports.runChainCheck = runChainCheck;
...
const CHAINS = [
    {
        ...
        steps: [
            ...
            {
                workshop: "Цех №4-2",
                output: "🪤[Каркас - напівфабрикат] (Д.Леон-Люкс Колеса, %Дно Каркасу%, %Колір Ламінату%)",
                requiredInputs: [
            ...
            {
                workshop: "Цех №6",
                output: "🪤🧽[Каркас + Поролон - напівфабрикат 2] (Угол Леон-Люкс 200 Колеса, %Диван Пружинний Блок%, %Дно Каркасу%, %Диван Розмір Бильця%, %Колір Ламінату%)",
                requiredInputs: [
    ...
    },
    {
        ...
        steps: [
            ...
            {
                workshop: "Цех №4-2",
                output: "🪤[Каркас - напівфабрикат] (143П, %Дно Каркасу%, %Колір Ламінату%)",
                requiredInputs: [
    ...
    },
    {
        ...
        steps: [
            {
                workshop: "Цех №3-1",
                output: "🧩[Ламінат - нарізані деталі] (Д.Леон-Люкс Колеса, %Колір Ламінату%)",
                requiredInputs: [],
            },
            {
                workshop: "Цех №4-2",
...
function isProductLine(line) {
    ...
    return ((EMOJI_PREFIX.test(t) && t.includes("[")) ||
        BRACKET_PREFIX.test(t) ||
        SOFA_PREFIX.test(t));
}
// ─── Parser ─────────────────────────────────────────────────────────────────
function parseDocument(fileLines) {
...
function normalizeProduct(s) {
    ...
    const productId = sepIdx >= 0
        ? inner.slice(0, sepIdx).trim()
        : inner.replace(/\).*/, "").trim();
    return s.slice(0, parenOpen + 1) + productId;
}
// ─── Find BOM block ──────────────────────────────────────────────────────────
function findBom(sections, workshopKeyword, outputContains) {
    ...
    for (const bom of sec.boms) {
        if (normalizeProduct(bom.output).includes(normRequired))
            return bom;
    }
    return null;
}
 ...
 */
function hasZeroConsumption(bom, fileLines) {
    let foundZero = false;
    ...
    for (let i = bom.outputLineIdx + 1; i < fileLines.length; i++) {
        ...
        if (t.startsWith("#"))
            break; // new section
        if (isProductLine(t))
            break; // next BOM block
        // Positive quantity: "- 1.5 m³" etc. (non-zero digit)
        if (/[-]\s*[1-9][\d.,]*\s*(m³|m²|m|кг|шт)[.]?\b/u.test(t))
            return false;
        // Zero quantity: "- 0 m³" etc.
        if (/[-]\s*0+([.,]0*)?\s*(m³|m²|m|кг|шт)[.]?\b/u.test(t))
            foundZero = true;
    }
    return foundZero;
}
// Returns indices of the output line + all non-empty content lines (materials, Ціна) of a BOM
function getBomContentLines(bom, fileLines) {
    const result = [bom.outputLineIdx];
    for (let i = bom.outputLineIdx + 1; i < fileLines.length; i++) {
        const t = fileLines[i].trim();
        if (!t)
            continue;
...
function isCommentedOutInFile(fileLines, normProduct) {
 ...
 */
function getBomAllLineIndices(bom, fileLines) {
    const result = [bom.outputLineIdx];
    for (let i = bom.outputLineIdx + 1; i < fileLines.length; i++) {
        const t = fileLines[i].trim();
        if (!t)
        ...
        if (t.startsWith("#"))
            break;
        if (/^(або|Або|АБО)$/.test(t))
            break;
        if (isProductLine(t) && !hasQty(t))
            break; // next output line
        result.push(i);
    }
    return result;
}
 ...
 */
function detectOrphans(sections, fileLines) {
    const issues = [];
    const fixes = [];
    const produced = [];
    const consumedNorms = [];
    ...
    const seen = new Set();
    const commentedLines = new Set(); // avoid double-commenting same line
    for (const out of produced) {
        const dedupeKey = `${out.section}::${out.norm}`;
        if (seen.has(dedupeKey))
            continue;
        const consumed = consumedNorms.some((c) => c.includes(out.norm) || out.norm.includes(c));
        if (!consumed) {
            seen.add(dedupeKey);
...
function detectProductIdMismatches(sections, fileLines) {
    const byType = new Map();
    const addOcc = (s, section, lineIdx, isOutput) => {
        if (!EMOJI_PREFIX.test(s))
            return;
    ...
    for (const [type, occs] of byType) {
        ...
        for (const outId of outputIds) {
            for (const inId of inputIds) {
                ...
                const isMismatch = inId.startsWith(outId + " ") || outId.startsWith(inId + " ");
                if (!isMismatch)
                    continue;
                ...
                if (/^[А-ЯҐЄІЇ]{1,4}$/.test(suffix))
                    continue;
                const key = `${type}::${outId}::${inId}`;
                if (reported.has(key))
                    continue;
                reported.add(key);
                const msg = `[BREAK] ${type}: productId не збігається — ` +
                ...
                for (const occ of occs.filter((o) => o.isOutput && o.productId === outId)) {
                    ...
                    if (!alreadyTodo) {
                        fixes.push({
                            lineIdx: occ.lineIdx,
                            content: `<!-- TODO: [BREAK] productId "${outId}" — у компоненті записано "${inId}". ` +
                ...
                for (const occ of occs.filter((o) => !o.isOutput && o.productId === inId)) {
                    if (occ.lineIdx < 0)
                        continue;
                    const alreadyTodo = fileLines
                        .slice(occ.lineIdx + 1, occ.lineIdx + 4)
                    ...
                    if (!alreadyTodo) {
                        fixes.push({
                            lineIdx: occ.lineIdx,
                            content: `<!-- TODO: [BREAK] productId "${inId}" — у заголовку записано "${outId}". ` +
                                `Привести до спільного вигляду скрізь у файлі -->`,
                            type: "insert-after",
                        });
                    }
...
function detectBrokenChains(sections, fileLines) {
    const issues = [];
    const fixes = [];
    function extractEntry(s, section, lineIdx) {
        if (!s)
            return null;
        ...
        if (RAW_MATERIAL_UNIT_RE.test(s))
            return null;
        const bracketType = getProductType(s);
        if (!bracketType)
            return null; // no bracket → sofa or service line
        const firstArg = getProductId(s);
        if (!firstArg)
            return null; // empty productId → template leftover (handled by detectOrphans)
    ...
    function hasTodoTag(lineIdx, tag) {
    ...
    const reportedOut = new Set();
    const reportedIn = new Set();
    function isFinishedGood(bracketType) {
        const t = bracketType.replace(/^[🪵🧩🪤🧽\s]+/u, "").trim();
        return t === "[Диван]" || t === "[Ліжко]";
    }
    // ─── [CHAIN-OUT]: produced but not consumed ──────────────────────────────
    for (const out of outputs) {
        if (reportedOut.has(out.key))
            continue;
        if (inputKeySet.has(out.key))
            continue; // correctly consumed → OK
        if (isFinishedGood(out.bracketType))
            continue; // sofa/bed is the chain end
        ...
        if (hasTodoTag(out.lineIdx, "[CHAIN-OUT]"))
            continue;
        if (hasTodoTag(out.lineIdx, "[BREAK] productId"))
            continue;
        ...
        const msg = `[CHAIN-OUT] ${out.section}: "${out.firstArg}" (${out.bracketType}) ` +
        ...
        console.log(`  ${msg}`);
        issues.push(msg);
        const todoText = consumedVariants.length
    ...
    for (const inp of inputs) {
        if (reportedIn.has(inp.key))
            continue;
        if (outputKeySet.has(inp.key))
            continue; // correctly produced → OK
        ...
        if (!outputBracketTypes.has(inp.bracketType))
            continue;
        // Prefix mismatch is detectProductIdMismatches territory — skip.
        ...
        if (hasTodoTag(inp.lineIdx, "[CHAIN-IN]"))
            continue;
        if (hasTodoTag(inp.lineIdx, "[BREAK] productId"))
            continue;
        ...
        const msg = `[CHAIN-IN] ${inp.section}: "${inp.firstArg}" (${inp.bracketType}) ` +
            `споживається але не знайдений як виріб. ` +
            `Очікував виробництво у: ${producedVariants.join(", ")}`;
        console.log(`  ${msg}`);
        issues.push(msg);
        const todoText = `<!-- TODO: [CHAIN-IN] споживається у ${inp.section}, але не знайдений як виріб. ` +
...
function verify(fileLines, sections) {
    const fixes = [];
    const issues = [];
    const knownOutputs = buildKnownOutputs();
    const issueSet = new Set(); // deduplicate fixes
    const zeroSourceScheduled = new Set(); // outputLineIdx of sources already scheduled for commenting
    for (const chain of CHAINS) {
        for (let si = 0; si < chain.steps.length; si++) {
            ...
            if (!bom) {
                console.warn(`  [WARN] BOM not found: ${step.workshop} → "${step.output}"`);
                continue;
            }
            for (const required of step.requiredInputs) {
                ...
                const activePresent = bom.inputs.some((inp) => normalizeProduct(inp).includes(normRequired));
                if (activePresent)
                    continue;
                ...
                if (sourceHasZero) {
                    ...
                    if (sourceBomToComment &&
                        ...
                        for (const lineIdx of contentLines) {
                            const orig = fileLines[lineIdx].trim();
                            const suffix = lineIdx === sourceBomToComment.outputLineIdx
                                ? " - уточнити списання"
                                : "";
                            fixes.push({
                                lineIdx,
                        ...
                        console.log(`  [ZERO] ${chain.name}: закоментовано BOM з нульовим списанням "${sourceBomToComment.output}"`);
                    ...
                    console.log(`  [ZERO] ${chain.name} @ ${step.workshop}: нульове списання "${required}" — закоментовано`);
                    ...
                    console.log(`  [BREAK] ${chain.name} @ ${step.workshop}: відсутній вхід "${required}"`);
    ...
    for (const [key, sec] of sections) {
        for (const bom of sec.boms) {
            for (let ii = 0; ii < bom.inputs.length; ii++) {
                ...
                const inChain = [...knownOutputs].some((out) => normInp.includes(normalizeProduct(out)));
                if (!inChain)
                    continue; // not related to any chain → skip
                // It's referenced in chains but does the output actually exist?
                const producedSomewhere = [...knownOutputs].some((out) => normInp.includes(normalizeProduct(out)));
                if (producedSomewhere)
                    continue;
                const rawLine = bom.rawInputLines[ii];
                ...
                if (issueSet.has(warnKey))
                    continue;
                issueSet.add(warnKey);
                console.log(`  [WARN] ${key}: вхід "${inp}" не зустрічався раніше в жодному ланцюгу`);
...
// ─── Apply fixes ─────────────────────────────────────────────────────────────
function applyFixes(fileLines, fixes) {
    if (fixes.length === 0)
        return fileLines;
 ...
 */
function isTodoAnnotation(content) {
    return /<!--\s*(TODO:|!!!)/i.test(content);
}
function runChainCheck(content, fileName, applyTodos = true) {
    console.log(`\nЛанцюги: ${fileName}`);
    ...
    const { issues: mismatchIssues, fixes: mismatchFixes } = detectProductIdMismatches(sections, fileLines);
    ...
    const { issues: chainIssues, fixes: chainFixes } = detectBrokenChains(sections, fileLines);
    ...
    if (usable.length > 0)
        console.log(`  Виправлень: ${usable.length}`);
    return {
