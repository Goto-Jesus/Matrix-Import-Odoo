...
const PLAIN_PRODUCT_ATTR_NAMES = {
    Диван: ["Тканина", "Диван Пружинний Блок", "Диван Наповнювач Подушек"],
};
function getAttrNames(bracketName, plainPrefix) {
...
function valuesToAttributes(values, bracketName, plainPrefix) {
...
function extractFirstOuterParen(s) {
...
function buildTemplateName(emoji, bracketName, isBracketed) {
    if (isBracketed)
        return `${emoji}[${bracketName}]`;
    return bracketName;
}
...
function embedLiteralFirstAttr(bracketName, attrValues) {
    if (EMBED_MODEL_TEMPLATES.has(bracketName) &&
        attrValues.length > 0 &&
...
function buildVariantDisplayName(templateName, attrValues) {
...
function tryParseProduct(line, isFirstInSection) {
    const { clean: trimmed, ifAttr } = stripIfAttrTag(line.trim());
    if (!trimmed || isSkipLine(trimmed))
        return null;
...
function parseDoc(content) {
    ...
    function flushBom() {
        if (!currentProduct || !currentWorkshop)
            return;
    ...
    for (const rawLine of lines) {
        ...
        if (trimmed.startsWith("Ціна")) {
            ...
            for (let bi = workshopBomStart; bi < boms.length; bi++) {
                ...
                if (ops.length > 0)
                    ops[ops.length - 1].priceRate = price;
            }
            continue;
        }
