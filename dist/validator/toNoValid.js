...
exports.toNoValidContent = toNoValidContent;
const ATTR_CATALOG = [
    {
        name: "Тканина",
        detect: /тканин/i,
...
function stripSyntaxPreamble(content) {
...
function titleCaseProduct(raw) {
    return raw
        ...
        if (!word)
            return word;
        if (/[А-ЯҐЄІЇA-Z]/.test(word.slice(1)))
            return word;
...
function parseExistingFlags(content) {
...
function inferFlags(notes, body) {
    const hay = `${notes}\n${body}`;
    const flags = {};
...
function splitHeadBody(content) {
...
function extractNotes(head) {
    return head
        ...
        if (!t)
            return false;
        if (/^диван\b|^ліжко\b/i.test(t))
            return false;
        if (/^# СПИСОК АТРИБУТІВ/i.test(t))
            return false;
        if (/^## Інструкції/i.test(t))
            return false;
        return true;
    })
        .join("\n");
...
function toNoValidContent(raw) {
    ...
    if (!hasInstructions)
        changes.push("додано шаблон «## Інструкції»");
    const header = [
        title,
        "",
